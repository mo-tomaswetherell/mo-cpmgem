...
def save_transform(xfm, path):
    """Write a transform object to a file."""
...
def load_transform(path):
    """Read a transform object from a file."""
...
class Standardize:
    ...
    # TODO: Is this correct? Shouldn't the transform be fit to the dataset used to
    # train the model - not the current dataset. In practise, you won't have a training split
    # to use to fit the transform outside of model training.
    def fit(self, target_ds, model_src_ds):
...
class RawMomentT:
    ...
    def fit(self, target_ds, model_src_ds):
        self.raw_moments = {
            var: np.power(np.mean(np.power(target_ds[var], self.root_base)), 1 / self.root_base)
            for var in self.variables
        }
...
def build_input_transform(variables, key="v1") -> ComposeT:
    if key == "v1":
